...
public class MyHashMap {
    private static int INIT_BUCKET_SIZE = 23;
    ...
    public MyHashMap() {
        array = new ListNode[MyHashMap.INIT_BUCKET_SIZE];
    }

    private void increase() {
        ListNode[] old = this.array;
        this.array = new ListNode[old.length * 2];
        for (ListNode p : old) {
            while (p != null) {
    ...
    private void _put(ListNode node) {
        ListNode p = this.array[this.getBucketLocation(node.key)];
        if (p == null || p.key.equals(node.key)) {
            this.array[this.getBucketLocation(node.key)] = node;
            node.next = p == null?p:p.next;
            return;
        }
        p = this.find(p, node.key);
        if (p == null) {
            node.next = this.array[this.getBucketLocation(node.key)];
            this.array[this.getBucketLocation(node.key)] = node;
            return;
        }
    ...
    public void put(Integer key, Integer value) {
        if (this.size >= MyHashMap.CAPACITY * this.array.length)
            this.increase();
        this._put(new ListNode(key, value));
        this.size++;
    }

    public Integer get(Integer key) {
        ListNode p = this.array[this.getBucketLocation(key)];
        if (p == null)
            return -1;
        if (p.key.equals(key))
            return p.val;
        p = this.find(p, key);
        return p == null ? -1 : p.next.val;
    }
    ...
    public void remove(Integer key) {
        ...
        if (p == null)
            return;
        if (p.key.equals(key))
            this.array[this.getBucketLocation(key)] = this.array[this.getBucketLocation(key)].next;
        p = this.find(p, key);
        if (p != null)
            p.next = p.next.next;
    }

    ListNode find(ListNode unit, int key) {
        ListNode p = unit;
